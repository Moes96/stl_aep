...
int main(int argc, char** argv)
{
  ...
  ros::Publisher pub(nh.advertise<geometry_msgs::PoseStamped>("/next_goal",
                                                              1000));     // /mavros/setpoint_position/local
  ...
  ros::Publisher goal_pub(nh.advertise<geometry_msgs::PoseStamped>("/next_goal", 1000));   ///mavros/setpoint_position/local
  ...
  ROS_INFO("Fly to action server started!");
  ...
  geometry_msgs::PoseStamped::ConstPtr init_pose =
      ros::topic::waitForMessage<geometry_msgs::PoseStamped>("/position_drone");   //   /mavros/local_position/pose
  ...
  ros::Publisher setpoint_pub =
      nh.advertise<geometry_msgs::PoseStamped>("/next_goal", 10);
  for (int i = 0; i < 50; i++)
  {
  ...
  // aggiunta per misurare tempo esecuzione e lunghezza trajectory
  ...
  while (ros::ok())
  {
    ...
    //aggiunta per tempo simulazione
    std::cout<<"Time of execution: "<<(ros::Time::now()-starttimer).toSec()<<"\n";
    //fine aggiunta
    ...
    if (aep_ac.getResult()->is_clear)
    {
      ...
      //aggiunta per calcolo lunghezza traiettoria
      double incr_x = goal_pose.pose.position.x - last_pose.pose.position.x;
      double incr_y = goal_pose.pose.position.y - last_pose.pose.position.y;
      double incr_z = goal_pose.pose.position.z - last_pose.pose.position.z;

      trajectory += std::sqrt((incr_x*incr_x)+(incr_y*incr_y)+(incr_z*incr_z));

      std::cout<<"Trajectory length: "<<trajectory<<"\n";
      //fine aggiunta
    ...
    else
    {
      ...
      {
        ROS_WARN("Exploration complete!"); //TODO add back to home instead of break************ 
        //aggiunta per misurare tempo
        std::cout<<"Final time of execution: "<<(ros::Time::now()-starttimer).toSec()<<"\n";
        //fine aggiunta
        break;
      }
